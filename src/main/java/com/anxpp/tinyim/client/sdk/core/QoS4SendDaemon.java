...
public class QoS4SendDaemon {
    ...
    private void init() {
        this.timer = new Timer(CHECK_INTERVAL, e -> QoS4SendDaemon.this.run());
    }
                ...
                for (String key : this.sentMessages.keySet()) {
                    ...
                    if ((p != null) && (p.isQoS())) {
                        if (p.getRetryCount() >= QOS_TRY_COUNT) {
                            ...
                            long delta = System.currentTimeMillis() - this.sendMessagesTimestamp.get(key);
    ...
    void startup(boolean immediately) {
        stop();
    ...
    void remove(final String fingerPrint) {
        new SwingWorker<Protocal, Object>() {
            ...
            protected void done() {
                ...
                Log.w(TAG, "【IMCORE】【QoS】指纹为" + fingerPrint + "的消息已成功从发送质量保证队列中移除(可能是收到接收方的应答也可能是达到了重传的次数上限)，重试次数=" + (result != null ? result.getRetryCount() : "none呵呵."));
            }
        }.execute();
    }
